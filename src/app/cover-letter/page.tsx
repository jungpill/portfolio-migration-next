    ...
    <CoverLetterContainer as="article" aria-labelledby="coverletter-title">
      ...
      <CoverLetterWrapper>
        ...
        <section aria-labelledby="sec-half">
          ...
          <Paragraph>
            회사에는 프론트엔드 개발자로 입사했지만, 업무를 진행하며 사내 서버 운영과 클라우드 관리에 개
선이 필요하다는 점을 발견했습니다. 불편함을 느끼는 데 그치지 않고, 현재 운영 환경에서 발생하는 
비용, 보안, 관리 측면의 문제를 정리해 대표님께 정식으로 개선 보고서를 작성하며 문제 원인과 개
선 방향을 구체화했고, 이를 계기로 서버 관리 업무까지 역할을 확장하게 되었습니다.
역할 전환 이후에는 먼저 AWS 비용 구조 개선을 진행하며 실사용량 대비 과도하게 할당된 인스턴스
와 유휴 리소스를 찾아 사이징을 조정하고, 상시 가동이 필요 없는 리소스를 정리하면서 AWS 월 운
영 비용을 60% 감소시켰습니다. 이후 GPU 서버 접근 보안을 강화하고 Prometheus, Grafana기반 
모니터링 환경을 구축하며, 장애나 이상 징후에 빠르게 대응할 수 있는 운영 체계로 개선했습니다
이러한 경험을 통해 인프라는 단순히 서버를 안정적으로 띄우는 일이 아니라, 비용과 리소스 효율까
지 함께 책임지는 영역이라는 것을 체감하였고, 안정적인 운영 환경을 유지하는 인프라 엔지니어로 
성장하기 위해 계속해서 개인 서버 환경에 VM과 Nginx, Docker를 활용해 배포 구조를 직접 구성하
며 운영 경험을 쌓고 있습니다. 앞으로도 문제를 발견하는 데 그치지 않고, 원인을 분석하고 개선 방
향을 실행으로 옮기며 안정성과 효율성을 함께 높일 수 있는 인프라 엔지니어로 성장하고 싶습니다
          </Paragraph>
        </section>
